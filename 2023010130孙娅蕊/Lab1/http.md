...
1. HTTP 请求报文由哪几部分构成？请按顺序列出：
   ...
   > 答：HTTP 请求报文由请求行、请求头、空行、请求体四部分构成。

2. 状态码 `404` 代表什么含义？状态码 `500` 和 `503` 有什么区别？

   > 答：404表示服务器无法找到请求的资源。500是服务器内部程序出错；503是服务器暂时无法处理请求，属于临时状态。

3. GET 与 POST 方法的主要区别是什么？各适用于什么场景？

   > 答：区别是GET 参数拼在 URL 中、长度受限、明文展示；POST参数放请求体、长度无严格限制、更安全。
适用场景：GET 适用于查询、获取数据，如浏览页面、搜索；POST 适用于提交、创建数据，如登录、上传文件、提交表单。

4. HTTP 与 HTTPS 有什么区别？HTTPS 使用了什么机制来保护数据？

   > 答：区别是HTTP 是明文传输，不安全；HTTPS 是 HTTP+SSL/TLS 加密，可验证身份，数据更安全。
保护机制：通过SSL/TLS 协议实现，结合非对称加密、对称加密、数字证书来保护数据。

5. 既然 HTTPS 已经加密，为什么浏览器开发者工具仍然能看到请求和响应的明文内容？

   > 答：浏览器开发者工具是在客户端展示请求、响应的解密后内容，HTTPS 加密是传输过程中加密，数据到浏览器端已解密，因此工具能看到明文，并非传输中未加密。
