...
| `server.py` 执行 `close()` 之后 | TIME-WAIT|连接关闭（TIME_WAIT）阶段 |
...
| :--- | :------- | :------- |
| 程序是否自动退出 |是 |否 |
| `ss` 中连接的 State |TIME-WAIT（连接关闭后） |ESTABLISHED（长连接保持） |
...
1. 实验中 `server.py` 必须比 `client.py` 先启动。如果顺序反过来，`client.py` 会报什么错误？用今天学到的概念解释：此时套接字存在吗？管道存在吗？
   ...
   > 答：会报 ConnectionRefusedError（连接被拒绝） 错误。此时客户端套接字存在，但服务器没有监听，所以连接管道不存在，无法建立通信。

2. `server.py` 打印出了两个不同的描述符编号（`server` 和 `conn`）。为什么 `accept()` 要返回一个新的套接字，而不是直接复用原来的 `server` 套接字？

   > 答：server 是监听套接字，专门负责等待客户端连接。conn 是连接套接字，专门负责和客户端收发数据。
分离职责，让一个服务器能同时处理多个客户端连接。

3. 描述符和端口号都可以用来标识一个套接字，它们的本质区别是什么？各自解决了什么问题？

   > 答：端口号：标识主机里的服务（如 80、8080），解决 “如何找到服务器”。描述符：是操作系统给当前进程的唯一编号，标识进程打开的套接字，解决 “进程如何管理连接”。

4. `client.py` 中，你有没有指定过客户端自己的端口号？这个端口号是谁分配的？在 `ss` 的输出中能看到它吗？

   > 答：没有指定。客户端端口由操作系统自动临时分配。在netstat 输出中能看到，就是客户端那一串数字端口。

5. HTTP/1.0 实验中，即使你没有调用 `close()`，`recv()` 循环也会自动退出。解释这是为什么？

   > 答：因为 HTTP/1.0 默认短连接，服务器发完数据会主动关闭连接，客户端 recv() 收到空数据，循环自动退出。

6. HTTP/1.1 实验中，程序卡在 `recv()` 不退出，`ss` 显示连接仍是 `ESTABLISHED`。这说明 HTTP/1.1 和 HTTP/1.0 在连接管理上有什么根本区别？

   > 答：说明 HTTP/1.1 默认使用长连接（keep-alive），连接不会自动关闭，会一直保持 ESTABLISHED，等待复用，所以程序卡在 recv()。
