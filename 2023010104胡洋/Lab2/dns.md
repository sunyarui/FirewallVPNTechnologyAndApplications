...
1. DNS 的作用是什么？为什么访问网站时通常要先进行 DNS 解析？
   ...
   > 答：DNS的作用是将人类可读的域名解析为机器可识别的 IP 地址。
   访问网站前必须进行 DNS 解析，因为网络层和传输层通信依赖 IP 地址定位主机，域名无法直接用于路由和连接建立。


2. 你抓到的 DNS 请求包和响应包中的 **Transaction ID** 是否一致？这个字段有什么作用？

   > 答:一致
   该字段是请求-响应匹配标识，用于客户端将收到的响应与之前发送的请求一一对应，避免多并发查询时的混淆

3. 这次实验中查询类型通常是 `A` 记录。`A` 记录表示什么含义？

   > 答：A记录表示将域名映射到 IPv4 地址的资源记录，是最基础的 DNS 记录类型之一。

4. DNS 默认通常使用哪个传输层协议和端口？在什么情况下可能会改用 TCP？

   > 答：默认使用UDP协议，端口为 53
   改用 TCP 的情况：
响应数据超过 512 字节（UDP 报文长度限制）
区域传输（Zone Transfer）场景
要求可靠传输的 DNSSEC 验证等场景

5. 为什么说 `gethostbyname()` 不是现代工程中推荐继续使用的接口？请至少写出两点原因。

   > 答：不支持 IPv6：仅返回 AF_INET 类型的 IPv4 地址，无法适配 IPv6 网络环境
线程不安全：内部使用静态缓冲区存储结果，多线程调用会导致数据竞争和覆盖
功能有限：无法获取 TTL、地址优先级等扩展信息，已被 getaddrinfo() 等现代接口替代

6. 如果一个域名返回了多个 IP 地址，这通常说明了什么？

   > 答：通常说明该域名部署了负载均衡或高可用架构，通过将流量分发到多个服务器来提升并发处理能力、容错能力和访问速度，也可能是 CDN 节点的分布
