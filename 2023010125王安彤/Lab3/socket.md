...
| :--- | :------------------ | :--------------------- |
| `server.py` 已启动，`client.py` 尚未运行 |LISTEN | 等待连接 / 监听阶段|
| `client.py` 已连接，数据尚未发送完毕 | ESTABLISHED| 数据传输阶段|
| `server.py` 执行 `close()` 之后 | TIME_WAIT| 连接关闭阶段|

**嵌入截图（LISTEN 状态）：**

![alt text](ss_listen.png)

**嵌入截图（ESTABLISHED 状态）：**

![alt text](ss_established.png)
...
| 响应的第一行（状态行） | 	HTTP/1.1 200 OK (截图第一行)|
| 程序是否自动退出（不需要手动 Ctrl+C）| 是 |
| 是你调用 `close()` 触发的断开，还是服务器主动断开的 |服务器主动断开的 (HTTP/1.0 默认请求完成后关闭) |

**嵌入截图：**

![alt text](http_run.png)
...
1. 实验中 `server.py` 必须比 `client.py` 先启动。如果顺序反过来，`client.py` 会报什么错误？用今天学到的概念解释：此时套接字存在吗？管道存在吗？
   ...
   > 答：会报 ConnectionRefusedError（连接被拒绝） 错误。
客户端的套接字（socket）是存在的，已经创建成功。
但是连接管道不存在，因为服务器没有启动、没有监听、没有接受连接。

2. `server.py` 打印出了两个不同的描述符编号（`server` 和 `conn`）。为什么 `accept()` 要返回一个新的套接字，而不是直接复用原来的 `server` 套接字？

   > 答：server 套接字专门负责监听连接请求，不负责收发数据。
conn 是专门和当前客户端通信的新套接字，用来收发消息。
分开的目的：一个监听，一个通信，服务器才能同时处理多个客户端。

3. 描述符和端口号都可以用来标识一个套接字，它们的本质区别是什么？各自解决了什么问题？

   > 答：描述符（fd）：是操作系统给当前进程分配的编号，只在本机有效，用来标识文件 / 套接字。
端口号：是网络上的地址编号，用来让其他电脑找到服务。

4. `client.py` 中，你有没有指定过客户端自己的端口号？这个端口号是谁分配的？在 `ss` 的输出中能看到它吗？

   > 答：客户端没有手动指定端口号。
端口由操作系统自动分配（临时端口 / 动态端口）。
使用 ss -tuln 或 ss -ta 命令能看到。

5. HTTP/1.0 实验中，即使你没有调用 `close()`，`recv()` 循环也会自动退出。解释这是为什么？

   > 答：因为 HTTP/1.0 服务器发送完数据后，会主动关闭连接。连接关闭后，recv() 会收到空数据 b''，循环就退出了。

6. HTTP/1.1 实验中，程序卡在 `recv()` 不退出，`ss` 显示连接仍是 `ESTABLISHED`。这说明 HTTP/1.1 和 HTTP/1.0 在连接管理上有什么根本区别？

   > 答：说明 HTTP/1.1 默认保持连接不断开（长连接），服务器不会主动关闭。所以 recv() 一直等待数据，连接保持 ESTABLISHED 状态。
